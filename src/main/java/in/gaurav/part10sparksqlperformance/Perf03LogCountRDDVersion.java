...
import static in.gaurav.common.SerializableComparator.serialize;

public class Perf03LogCountRDDVersion {
    public static void main(String[] args) {
        Logger.getLogger("org.apache").setLevel(Level.WARN);

        SparkConf conf = new SparkConf().setMaster("local[*]").setAppName("startingSpark");
        JavaSparkContext sc = new JavaSparkContext(conf);
        JavaRDD<String> input = sc.textFile("src/main/resources/newbiglog.txt");
    ...
    private static int monthToMonthnum(String month) {
        ...
        try {
            ...
            int monthNum = Integer.parseInt(requiredFmt.format(results));
            return monthNum;
        } catch (ParseException e) {
            throw new RuntimeException(e);
        }
